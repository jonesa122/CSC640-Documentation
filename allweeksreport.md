# HW5 – Quality Software Documentation
**Author:** Abby Jones  
**Course:** CSC 640 – Fall 2025  

## Abstract
This report explains the principles of quality software using HW4 as an example project. It covers tools, rules, and practices that ensure reliability, maintainability, and scalability.
...
Even though the final deadline is November 7th, breaking the work into smaller milestones with planned dates helped me stay on track and avoid last-minute stress.
...
OOP organizes software around objects — bundles of data and behavior that model real-world things.  
Its core pillars provide powerful benefits:

- **Encapsulation** → Keeps internal details private, exposing only what is necessary  
...
This separation was key to good design: each module encapsulates its own responsibilities, while the interfaces expose only what is necessary for other parts of the system to interact with it cleanly and predictably.
...
## 🐾 HW4 Example: Animal Shelter API (Debugging Philosophy)  

Errors surface clearly through endpoints.  
For instance, if an adoption request references a non-existent animal, the API returns a clear error message.  
This design makes problems visible instead of hidden.  
...
# 📚 Overall Conclusion: Bringing It All Together  

First, we focused on **managing complexity** — keeping things simple and predictable so teams don’t get lost in their own code. From there, we saw how **milestones in software development** and evolving models help projects stay on track and adapt when change inevitably arrives.  

As the weeks progressed, we turned to **designing with purpose**. Object-oriented principles, UML diagrams, and modular thinking showed how to create systems that are reusable and easy to maintain. Then, we dug deeper into **advanced design principles**, learning why composition often beats inheritance, how loose coupling makes systems more resilient, and how the SOLID principles keep code safe to change.  

By Week Five, we explored **software evolution & architecture**. Anticipating risks early, separating concerns with MVVM, and even fostering team happiness all contribute to long-term success. Finally, in Week Six, we grounded everything in **software engineering in practice**: weighing buy vs build decisions, distinguishing architecture from design, applying design patterns, and using strong type systems to catch errors before they cause trouble.  

The HW4 Animal Shelter API gave me a concrete way to put these lessons into practice. Through simple, predictable endpoints, milestone-driven progress, and modular design, I was able to see how theory translates into real, working software.  

In the end, the takeaway is clear: quality software doesn’t come from one principle alone. It comes from combining simplicity, structure, adaptability, and practicality — and from making choices that keep both the system and myself in a good place as projects grow and change.  
